...
;(() => {
  ...
  function stopV5Timer(){
    if (v5Timer) clearInterval(v5Timer);
    v5Timer = null;
  }
  function startV5CountdownLoop(){
    stopV5Timer();
    const tick = () => {
      document.querySelectorAll("[data-v5end]").forEach(el => {
        const end = el.getAttribute("data-v5end");
        el.textContent = fmtCountdown(end);
      });
    };
    tick();
    v5Timer = setInterval(tick, 1000);
  }
  ...
  async function copyText(textRaw) {
    ...
    try {
      ...
      console.warn("clipboard.writeText blocked:", e);
      // ไป fallback
    }
  ...
  async function shareLink(url) {
    ...
    if (!u || u === "-") { toast("No link", "err"); return; }
    try {
      if (navigator.share) {
        await navigator.share({ title:"365DF Referral", text:"Join via my referral link", url: u });
        toast("Shared ✅");
      } else {
        await copyText(u);
      }
      ...
      for (let i=0;i<count;i++){
        ...
        if (wrap) {
          ...
          div.innerHTML = `
            ...
            <div class="kv"><span>Countdown</span><span class="mono" data-v5end="${end}">-</span></div>
  ...
  window.addEventListener("load", () => {
    ...
    // ✅ Copy/Share: ทำเป็น async + await + กันนิ่งใน DApp browser
    $("btnCopyLeft")?.addEventListener("click", async (e) => {
      e.preventDefault(); e.stopPropagation();
      await copyText($("leftLink")?.textContent || "");
    });
    $("btnCopyRight")?.addEventListener("click", async (e) => {
      e.preventDefault(); e.stopPropagation();
      await copyText($("rightLink")?.textContent || "");
    });
    $("btnShareLeft")?.addEventListener("click", async (e) => {
      e.preventDefault(); e.stopPropagation();
      await shareLink($("leftLink")?.textContent || "");
    });
    $("btnShareRight")?.addEventListener("click", async (e) => {
      e.preventDefault(); e.stopPropagation();
      await shareLink($("rightLink")?.textContent || "");
    });
